...
import {
	...
	type InstructorSignupData
} from '$src/features/Instructors/lib/instructorSchemas';
...
export const actions: Actions = {
	default: async (event) => {
		...
		console.log(
			'[Instructor Signup] Profile image size:',
			form.data.profileImage?.size || 0,
			'bytes'
		);
		console.log(
			'[Instructor Signup] Qualification size:',
			form.data.qualification?.size || 0,
			'bytes'
		);
		...
		try {
			...
			await funnelEventService.track(event, {
				eventType: 'supply.instructor_signup_completed',
				funnel: 'supply',
				stage: 'signup_completed',
				userId: user.id,
				entityType: 'user',
				entityId: user.id,
				countryCode: form.data.professionalCountryCode,
				metadata: {
					instructorType: form.data.instructorType,
					resortId: form.data.resort ?? null,
					sportsCount: Array.isArray(form.data.sports) ? form.data.sports.length : 0,
					spokenLanguagesCount: Array.isArray(form.data.spokenLanguages)
						? form.data.spokenLanguages.length
						: 0
				}
			});
		} catch (error) {
