...
export const load: PageServerLoad = async ({ parent }) => {
    ...
    // Get the base lesson for the school
    let baseLesson = null;
    let groupTiers: any[] = [];
    ...
    try {
        const lessons = await lessonRepository.listLessonsBySchool(school.id);
        baseLesson = lessons.find(lesson => lesson.isBaseLesson) ?? null;

        if (baseLesson) {
            // Load pricing structures
        ...
        console.error('Error fetching school lessons and pricing:', error);
    }
...
export const actions: Actions = {
    saveBaseLesson: async (event) => {
        ...
        try {
            // Check if school already has a base lesson
            const existingLessons = await lessonRepository.listLessonsBySchool(school.id);
            ...
            const lessonData = {
                ...
                schoolId: school.id, // Use schoolId instead of instructorId
                instructorId: null,
                isPublished: true,
                isBaseLesson: true,
            };
            ...
            console.error('Error saving school base lesson:', error);
            return fail(500, {
                form,
    ...
    createPromoCode: async (event) => {
        ...
        try {
            await db.insert(promoCodes).values({
                lessonId,
                instructorId: null, // School promo codes don't have instructorId
