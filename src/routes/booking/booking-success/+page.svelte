...
<svelte:head>
	<title>{$t('booking_success_page_title')} - Local Snow</title>
...
<section class="mx-auto max-w-2xl py-12">
	<div class="rounded-lg border border-border bg-card p-8 text-center">
		...
		<!-- Success Message -->
		<h1 class="title2 mb-4">{$t('booking_success_heading')}</h1>
		...
		<div class="mb-8 rounded-lg bg-muted/50 p-6 text-left">
			<h2 class="mb-4 text-lg font-semibold">{$t('booking_success_next_steps')}</h2>
			<ol class="space-y-3 text-sm text-muted-foreground">
				<li class="flex items-start gap-3">
					...
					<span>{$t('booking_success_step1')}</span>
				</li>
				<li class="flex items-start gap-3">
					...
					<span>{$t('booking_success_step2')}</span>
				</li>
				<li class="flex items-start gap-3">
					...
					<span>{$t('booking_success_step3')}</span>
				</li>
				<li class="flex items-start gap-3">
					...
					<span>{$t('booking_success_step4')}</span>
				</li>
			</ol>
		</div>
		...
		<div class="mb-6 rounded-lg border-2 border-blue-200 bg-blue-50 p-4 text-left">
			<div class="flex items-start gap-3">
				...
				<div class="text-sm text-blue-900">
					<p class="font-semibold mb-1">{$t('booking_success_deposit_title')}</p>
					<p>{$t('booking_success_deposit_info')}</p>
				</div>
			</div>
		</div>
		...
		<div class="flex flex-col gap-3 sm:flex-row sm:justify-center">
			...
			<Button onclick={() => window.location.href = '/instructors'} variant="outline">
				{$t('button_browse_more_instructors')}
			</Button>
		</div>
