...
export async function GET(event: RequestEvent): Promise<Response> {
	...
	// ADD THIS: Get the stored return URL
	const returnTo = event.cookies.get("oauth_return_to") ?? "/dashboard";
	...
	const googleUser = {
		...
		username: username,
		lastName: claims.family_name ? claims.family_name : "",
		email: claims.email,
	...
	if (existingUser !== null) {
		...
		// Add auth_success parameter to prevent redirect loop
		const redirectUrl = new URL(returnTo, event.url.origin);
