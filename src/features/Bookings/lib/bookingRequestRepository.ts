...
export class BookingRequestRepository {
	...
	async updateRequestStatus(id: number, status: BookingRequestStatus) {
	...
	async getBookingRequestSports(bookingRequestId: number): Promise<number[]> {
		const results = await db
			.select({ sportId: bookingRequestSports.sportId })
	...
	async updateBookingStatus(bookingRequestId: number, status: BookingRequestStatus) {
		const result = await db
			.update(bookingRequests)
			.set({
				status,
	...
	async getActiveRequestCount(
		...
		const result = await db
			.select()
			.from(bookingRequests)
			.where(and(clientCondition, inArray(bookingRequests.status, ['pending', 'viewed'])));
