...
LocalSnow can charge the client for protected booking/support while instructor/school payout remains manual until transaction volume justifies automation. The paid promise is lesson-outcome guarantee, not cheapest price or exact-instructor certainty: LocalSnow confirms the requested instructor first, then reschedules, replaces, or refunds if needed. If a replacement costs more, the client approves the difference before the booking changes.
```
